...
type StepsPanelProps = {
  ...
  setRecording: (updater: (value: boolean) => boolean) => void;
...
export function StepsPanel({
  ...
  return (
    <aside className="steps-panel">
      <div className={`panel automation-workbench ${activeTab === "runs" ? "runs-workbench" : "steps-workbench"}`}>
        {activeTab === "steps" ? (
          <div className="automation-tab-body steps-tab">
        <div className="panel-head">
          <h2>录制步骤</h2>
          <div className="toolbar-actions">
            <button className={recording ? "record-button active" : "record-button"} onClick={() => setRecording((value) => !value)}>
              <Video size={17} />
              {recording ? "录制中" : "开始录制"}
            </button>
            <button className="icon-button" onClick={() => void saveCase()} disabled={!steps.length} title="保存当前录制步骤为用例">
              <Save size={18} />
              {selectedCaseId ? "更新" : "保存"}
            </button>
            <button className="icon-button" onClick={resetEditor} disabled={!steps.length && !selectedCaseId} title="新建空白用例">
              <Trash2 size={18} />
              新建
            </button>
          </div>
