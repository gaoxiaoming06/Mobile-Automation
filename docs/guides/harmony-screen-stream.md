...
This is an experimental server/agent bridge, not the current default dashboard
preview path. The dashboard currently uses screenshot polling for HarmonyOS
devices; it only opens the browser stream URL for Android scrcpy.

When the bridge is enabled and exercised directly, it is split into two
channels:
...
The server and dashboard do not call HDC directly. The intended bridge flow is:

1. A client opens `/api/devices/:deviceKey/harmony-stream/ws`.
...
5. A WebCodecs client decodes the H.264 packets with the existing preview renderer.

## Agent Configuration

Default parameters once the bridge is enabled:
...
Opt in explicitly when testing the bridge:

```bash
export HARMONY_STREAM_ENABLED=1
```
...
The device-agent advertises `capabilities.harmonyScreenStream=true` only when `HARMONY_STREAM_ENABLED=1` and the HarmonyOS device is at or above `HARMONY_STREAM_MIN_API`. This capability is for the experimental stream endpoint; the current dashboard preview path still uses screenshots for HarmonyOS.
